...
object StyleShowcasePanel:
  ...
  private val combinations: Seq[(String, CellStyle)] = Seq(
    ...
    "Bold + Underline + Cyan" -> CellStyle(
      fg = Foreground.Named(FgColor.Cyan),
      attributes = Set(Attribute.Bold, Attribute.Underline)
    ),
    "Dim + Italic + Magenta" -> CellStyle(
      fg = Foreground.Named(FgColor.Magenta),
      attributes = Set(Attribute.Dim, Attribute.Italic)
    ),
    "Bold + Yellow + Underline + Strikethrough" -> CellStyle(
      fg = Foreground.Named(FgColor.BrightYellow),
      attributes = Set(Attribute.Bold, Attribute.Underline, Attribute.Strikethrough)
    ),
    "Italic + Underline + Green" -> CellStyle(
      fg = Foreground.Named(FgColor.BrightGreen),
      attributes = Set(Attribute.Italic, Attribute.Underline)
    )
  )
