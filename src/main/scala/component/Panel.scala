 ...
 * `style`), then the border on the outer edge (short-circuited when
 * `sides` is empty), then `child` into
 * `area.inner(sides.toInsets).inner(padding)`. Each visible edge costs
 * one cell along that edge; suppressed edges cost nothing. A panel with
 * `sides = Sides.none` and zero padding therefore hands the child the
 * panel's full area.
 *
...
final case class Panel(
  ...
  /** True when the area is too small to fit this panel's visible border edges. */
  ...
  override def childLayouts(area: Rect): Seq[(Component, Rect)] =
    ...
    if area.isEmpty || undersizedForBorder(area) then
      ...
      Seq((child, area.inner(sides.toInsets).inner(padding)))
  ...
  override def render(area: Rect, canvas: Canvas, ctx: RenderContext): Unit =
    ...
    canvas.drawBox(area, border, sides, title, style)
