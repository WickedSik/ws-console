 ...
 *   2. **Draw** — invoke `Component.render` to write cells into the
 *      current buffer.
    ...
    new RenderPipeline:
      def runFrame(root: Component, area: Rect): ZIO[Frame, IOException, LayoutResult] =
        ZIO.serviceWithZIO[Frame] { frame =>
          ...
          ZIO.succeed(drawPhase(root, area, canvas)) *>
            frame.render.as(layout)
        }

      private def drawPhase(root: Component, area: Rect, canvas: Canvas): Unit =
        if !area.isEmpty then root.render(area, canvas)
