...
**Style is one merged escape, and it is structural.** `CellStyle.sgr` emits every attribute and colour as a single SGR sequence: bold + yellow is `ESC[1;33m`, never `ESC[1m` followed by `ESC[33m`. `BufferFlusher` folds the per-cell reset into it too, so a styled cell carries `ESC[0;1;33m`. Two consequences for specs. First, a substring search for one component's escape (`FgColor.Red.toAnsi`) will not match inside a merged sequence — search for the composed `Sgr`, or better, decode. Second, the ordering is now canonical (`Attribute` declaration order, then fg, then bg), so byte-level comparison is at least *stable* — this closed KI-001, see `docs/known-issues.md`. Prefer structural comparison anyway: `AnsiGrid.decode` rebuilds the parameter list into a `CellStyle` value and `GridAssertions.assertStyle` compares that, which survives a change to emission order.
