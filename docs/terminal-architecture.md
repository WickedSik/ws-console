...
**Amendment (2026-08-02) — `Sgr`.** Layer 1 gained `ansi.Sgr`, which holds a Select Graphic Rendition sequence as its *parameter list* rather than its rendered bytes. SGR is the only CSI family accepting an unbounded parameter list, so it is the only one where several changes collapse into one escape; cursor, erase, scroll and mode sequences each own a distinct final byte and cannot merge. Composition is `++`, rendering happens once at the wire edge, and `Sgr.Empty` renders to the empty string rather than to a bare `ESC[m` (which would mean "reset all"). `Style`, `StyleDisable` and `Color.Templates` are now thin rendered facades over `Sgr` constants, so their types and values are unchanged. `AnsiBuilder.sgr(Sgr)` is the merge-aware append; the older per-attribute chaining methods (`.bold.fg(...)`) still emit one escape each and remain correct.

Downstream, `buffer.CellStyle.sgr` composes attributes (in `Attribute` declaration order), then foreground, then background, and `BufferFlusher` prefixes `Sgr.Reset` so a styled cell carries one escape (`ESC[0;1;33m`) instead of three. Measured: the SGR portion of a bold + italic + fg-RGB + bg-RGB cell drops 45 bytes to 37; a fully styled 80×24 repaint drops roughly 18%. An unstyled cell is byte-identical to the pre-merge flusher. The canonical attribute traversal also closed KI-001 (`docs/known-issues.md`). Cross-op style coalescing — not re-emitting an unchanged style between adjacent cells — remains deferred: it would overturn `BufferFlusher`'s stateless contract and `AnsiGrid`'s per-`H` style reset, and wants a benchmark first.
